    ...
    function initialize() {
        ...
        elements.userForm.list.addEventListener('click', e => {
            ...
            if (button.classList.contains('activate-user-btn')) {
                ...
                const userToDelete = state.resguardantes[index];
                
                // --- NUEVA VERIFICACIÓN ---
                const assignedInventoryItems = state.inventory.filter(item => item['NOMBRE DE USUARIO'] === userToDelete.name).length;
                const assignedAdditionalItems = state.additionalItems.filter(item => item.usuario === userToDelete.name).length;
                const totalAssigned = assignedInventoryItems + assignedAdditionalItems;

                const performDelete = () => {
                    const recentlyDeleted = { item: userToDelete, originalIndex: index };
                    state.resguardantes.splice(index, 1);
                    if (state.activeResguardante?.id === userToDelete.id) state.activeResguardante = null;
                    
                    // Marcar bienes como "Sin Asignar"
                    state.inventory.forEach(item => {
                        if (item['NOMBRE DE USUARIO'] === userToDelete.name) {
                            item['NOMBRE DE USUARIO'] = ''; 
                        }
                    });
                    state.additionalItems.forEach(item => {
                        if (item.usuario === userToDelete.name) {
                    ...
                    renderUserList();
                    populateUserSelects();
                    filterAndRenderInventory(); // Actualiza la tabla de inventario
                    renderAdicionalesList(); // Actualiza la lista de adicionales
                    logActivity('Usuario eliminado', `Nombre: ${userToDelete.name}. ${totalAssigned} bienes fueron desasignados.`);
                    
                    showUndoToast('Usuario eliminado.', () => {
                        state.resguardantes.splice(recentlyDeleted.originalIndex, 0, recentlyDeleted.item);
                        // NOTA: La acción de deshacer no reasigna los bienes automáticamente para mantener la lógica simple.
                        renderUserList(); saveState(); showToast('Acción deshecha.');
                        logActivity('Acción deshecha', `Restaurado usuario eliminado: ${userToDelete.name}`);
                    });
                    saveState();
                };
                ...
                if (totalAssigned > 0) {
                    showConfirmationModal(
                        'Usuario con Bienes Asignados',
                        `¡Atención! El usuario "${userToDelete.name}" tiene ${totalAssigned} bien(es) a su cargo. Si lo eliminas, estos bienes quedarán "Sin Asignar". ¿Deseas continuar?`,
                        performDelete,
                        { confirmText: 'Sí, Eliminar' }
                    );
                } else {
                    // Si no tiene bienes, eliminar directamente
                    performDelete();
                }
            }
        });
